...
@Component({
  ...
  templateUrl: './create-inventory-movement.component.html',
  styleUrls: ['./create-inventory-movement.component.scss']
})
export class CreateInventoryMovementComponent implements OnInit {
  ...
  getinfo(){
    ...
    async getAllItems() {
      debugger;
      let resGetAllItems = await this.itemService.getallItems().toPromise();
  ...
  onSubmit(): void {
    ...
    let inventoryVm: InventoryMovtVm = {
      ...
      this.inventorymovement.createInventoryMovt(inventoryVm).subscribe(
        {
          next: (data => {
