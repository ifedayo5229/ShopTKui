...
@Component({
  selector: 'app-direct-issue',
  templateUrl: './direct-issue.component.html',
  styleUrls: ['./direct-issue.component.scss']
})
export class DirectIssueComponent implements OnInit {
  ...
  ) 
  {
    this.itemRequestForm = this.fb.group({
      ...
      employeeLocationId: ['', Validators.required],
      employeeFunctionId: ['', Validators.required],
  ...
  getinfo(){
    ...
    async getITInventoryByStoreId(id: number) {
    ...
    async getAllStoreLocations() {
      ...
      this.firstFilteredStoreLocations = this.allStoreLocations;
    ...
    loadResources(){
      this.addItem(); 
      this.getinfo();
      this.getITInventoryByStoreId(this.storeId);
  ...
  filterLocations(searchTerm: string) {
    if (!searchTerm) {
      ...
      this.filteredLocations = this.filteredLocations.filter(store =>
        store.name.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
  }
  ...
  filterDepartments(searchTerm: string) {
    if (!searchTerm) {
      ...
      this.filteredDepartments = this.filteredDepartments.filter(store =>
        store.name.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
  }
  ...
  setupDepartmentSearch() {
    this.searchInputDepartments.valueChanges.subscribe(value => {
  ...
  async onStoreChange(storeId: number): Promise<void> {
    ...
    console.log(this.storeFunctionId)
    this.selectedStoreId = this.selectedStoreLocation.id;

   
    this.getITInventoryByStoreId(this.selectedStoreId);
  ...
  onSubmit(): void {
    ...
    console.log(this.itemRequestForm.value);
    ...
    let itemdisburseVm: DirectIssue = {
      userId: this.userId,
      employeeEmail: this.itemRequestForm.get('employeeEmail')?.value || '',
      employeeLocationId: this.itemRequestForm.get('employeeLocationId')?.value || '', 
      employeeFunctionId: this.itemRequestForm.get('employeeFunctionId')?.value || '',
      description: this.itemRequestForm.get('description')?.value || '',
      storeFunctionId: this.storeFunctionId,
      sourceStoreId : this.selectedStoreId,
      issueItems: (this.itemRequestForm.get('items') as FormArray)?.value || []
    }
  
      console.log(itemdisburseVm);
      debugger;
      this.inventoryMovementService.directIssues(itemdisburseVm).subscribe(
        {
          next: (data => {
