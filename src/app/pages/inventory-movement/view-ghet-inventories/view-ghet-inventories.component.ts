...
@Component({
  selector: 'app-view-ghet-inventories',
  templateUrl: './view-ghet-inventories.component.html',
  styleUrls: ['./view-ghet-inventories.component.scss']
})
export class ViewGhetInventoriesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.getinfo();
    this.getGhetInventoryMovts(this.storeId);
  ...
  async getGhetInventoryMovts(id: number) {
    let resGetInwardMovtsByStoreId = await this.inventoryMovementService.getGhetInventoryMovementsByStoreId(id).toPromise();
