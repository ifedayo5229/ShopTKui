...
@Component({
  selector: 'app-ghet-approve-dialog',
  templateUrl: './ghet-approve-dialog.component.html',
  styleUrls: ['./ghet-approve-dialog.component.scss']
})
export class GhetApproveDialogComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<GhetApproveDialogComponent>,
  ...
  async getRequestBy(id: number) {
            debugger
            let resGetRequest = await this.requestService.getGhetRequestBy(id).toPromise();
  ...
  approveRequest() {
    ...
    if (this.approveRequestForm.valid) {
      ...
      this.requestService.approveGhetRequest(approvalRequest).subscribe({
        next: (data) => {
          debugger;
