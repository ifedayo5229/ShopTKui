...
export class CreateItemComponent implements OnInit {
  ...
  getinfo(){
    ...
    //   itemGroup.get('costPerUnit')?.setValidators([
  ...
  const itemGroup = this.fb.group({
    ...
    description: ['', Validators.required],
    itemName: ['', Validators.required],
    costPerUnit: [null, this.optionalCurrencyValidator()] // ✅ start as null
  });

  this.items.push(itemGroup);
}

// ✅ Custom wrapper validator to skip empty values
private optionalCurrencyValidator() {
  return (control: any) => {
    const value = control.value;

    // ✅ allow empty/null values without error
    if (value === null || value === '' || value === undefined) {
      return null;
    }

    // ✅ delegate to your existing validator
    return this.validationService.validateCurrency()(control);
  };
}
  ...
  this.isSubmitting = true;
    ...
    let createItemRequestVm: CreateItem = {
      ...
      debugger;
      this.itemService.createItems(createItemRequestVm).subscribe(
        {
          next: (data => {
